...
#     parquet_file = pq.ParquetFile(inputfile)
#     try:
#         print(f"Debug: parquet_file.metadata: {parquet_file.metadata}")
#         print(f"Debug: parquet_file.metadata.num_rows: {parquet_file.metadata.num_rows}")
#         print(f"Debug: type(parquet_file.metadata.num_rows): {type(parquet_file.metadata.num_rows)}")
...
#             raise TypeError(f"total_rows is not an integer: {type(total_rows)}")

#     except AttributeError as e:
#         raise AttributeError("Could not determine total rows from Parquet metadata.") from e
...
#     # Determine the row group index
#         row_group_index = start // parquet_file.metadata.row_group(0).num_rows

#         # Read row group
#         row_group = parquet_file.read_row_group(row_group_index, columns=required_columns)
