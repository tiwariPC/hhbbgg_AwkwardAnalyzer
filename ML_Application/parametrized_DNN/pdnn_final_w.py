...
# Modified: sigmoid output layer, AMP-safe BCELoss
# Shivam Raj
# ---------------------
...
def predict_batched(model, X_tensor, device, batch=32768, use_amp=True):
    """
    Model is assumed to output probabilities in [0,1] (final Sigmoid layer).
    """
    model.eval()
    N = X_tensor.shape[0]
    ...
    with amp_ctx:
        for i in range(0, N, batch):
            ...
            probs = model(xb).view(-1)  # already sigmoid-ed
            out[i:i+batch] = probs.detach().cpu().numpy()
    return out
...
class ParameterizedDNN(nn.Module):
    """
    Final layer is sigmoid -> outputs probabilities in [0,1].
    """
    def __init__(self, d):
        super().__init__()
...
with torch.no_grad():
    p0 = model(X_va_t_cpu).view(-1).numpy()  # already sigmoid output
...
# -----------------------------
# Model outputs probabilities -> use BCELoss (probability space).
...
for epoch in range(MAX_EPOCHS):
    ...
    for bi, (xb, yb, wb) in enumerate(train_loader):
        ...
        # Forward pass under autocast (safe)
        with torch.amp.autocast(device_type=device.type, enabled=use_amp):
            probs = model(xb).view(-1)  # sigmoid output

        # Compute BCELoss outside autocast in full precision
        probs = probs.float()
        yb_f = yb.float()
        per_loss = criterion(probs, yb_f)
...
# -----------------------------
# 14) Test + diagnostics & plots (with CMS palette) — second block
# -----------------------------
if os.path.exists("GATE_ABORTED"):
    print("[INFO] Consistency gate aborted training early; proceeding with saved snapshot.")
...
for m, yv in uniq_groups:
    ...
    if np.unique(y_te[idx]).size < 2:
        continue
    fpr_g, tpr_g, _ = roc_curve(y_te[idx], test_probs[idx],
                                sample_weight=(w_te[idx] if w_te is not None else None))
    auc_g = auc(fpr_g, tpr_g)
...
def gradient_saliency(model, X_full, feature_names_report, device, scaler=None, batch=4096):
    ...
    while ptr < N:
        ...
        logits = model(xb).view(-1)  # note: technically probs, but derivative scaling is still fine for ranking
        s = logits.sum()
        s.backward()
